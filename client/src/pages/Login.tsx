...
export default function Login() {
  const { user, loading, error, login, loginWithOAuth } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      // Redirect based on role - user from Supabase has metadata, but we need app role
      // For now, redirect to home and let the app determine role
      navigate("/");
    }
  }, [user, navigate]);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-card rounded-lg p-8 shadow-sm border border-border">
          ...
          <p className="text-muted-foreground mb-6">
            Secure access to patient records and clinic operations
          </p>
          ...
          <div className="mt-8 pt-8 border-t border-border">
            <p className="text-sm text-muted-foreground text-center">
              Authorized personnel only. All access is logged for security and
              compliance purposes.
            </p>
          </div>
        </div>
